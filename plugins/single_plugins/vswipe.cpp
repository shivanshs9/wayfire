...
class vswipe : public wf::plugin_interface_t
{
  private:
    ...
    wf::option_wrapper_t<bool> smooth_transition{
  ...
  public:
    ...
    wf::signal_callback_t on_swipe_update = [&](wf::signal_data_t* data) {
        ...
        auto ev =
            static_cast<event<wlr_event_pointer_swipe_update>*>(data)->event;
        ...
        if (state.direction == UNKNOWN) {
            ...
            state.initial_deltas.x += std::abs(ev->dx) / speed_factor;
            state.initial_deltas.y += std::abs(ev->dy) / speed_factor;

            bool horizontal =
                state.initial_deltas.x > initial_direction_threshold;
            bool vertical = state.initial_deltas.y > initial_direction_threshold;
        ...
        if (state.direction == HORIZONTAL) {
            current_delta_processed = vswipe_process_delta(
                ev->dx, smooth_delta, state.vx, state.vw, cap, fac);
            state.delta_last = ev->dx;
        } else {
            current_delta_processed = vswipe_process_delta(
                ev->dy, smooth_delta, state.vy, state.vh, cap, fac);
    ...
    wf::signal_callback_t on_swipe_end = [=](wf::signal_data_t* data) {
        ...
        switch (state.direction) {
            case UNKNOWN:
                ...
                break;
            case HORIZONTAL:
                target_delta = vswipe_finish_target(smooth_delta.end, state.vx,
                    state.vw, state.delta_prev + state.delta_last, move_threshold,
                    fast_threshold);
                target_workspace.x -= target_delta;
                break;
            case VERTICAL:
                target_delta = vswipe_finish_target(smooth_delta.end, state.vy,
                    state.vh, state.delta_prev + state.delta_last, move_threshold,
                    fast_threshold);
    ...
    void fini() override
    {
        ...
        wf::get_core().disconnect_signal(
            "pointer_swipe_update", &on_swipe_update);
