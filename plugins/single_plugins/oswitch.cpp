...
class wayfire_output_manager : public wf::plugin_interface_t
{
    ...
    wf::activator_callback switch_output = [=](wf::activator_source_t, uint32_t) {
        ...
        auto next = wf::get_core().output_layout->get_next_output(output);
        idle_next_output.run_once([=]() { wf::get_core().focus_output(next); });

        return true;
    };

    wf::activator_callback switch_output_with_window = [=](wf::activator_source_t,
                                                           uint32_t) {
        auto next = wf::get_core().output_layout->get_next_output(output);
