...
#define MOVE_HELPER view->get_data<wf::move_snap_helper_t>()
  ...
  public:
    void init() override
    {
        ...
        output->add_touch(wf::create_option_string<wf::keybinding_t>("<super>"),
    ...
    bool initiate(wayfire_view view)
    {
        ...
        view->store_data(
            std::make_unique<wf::move_snap_helper_t>(view, get_input_coords()));
    ...
    void update_slot(int new_slot_id)
    {
        ...
        if (new_slot_id) {
            ...
            auto input = get_input_coords();
            auto preview = new wf::preview_indication_view_t(
                output, {input.x, input.y, 1, 1});
    ...
    void delete_mirror_view_from_output(
        wf::output_t* wo, bool show_animation, bool already_unmapped)
    {
    ...
    void delete_mirror_views(bool show_animation)
    {
        for (auto& wo : wf::get_core().output_layout->get_outputs()) {
            delete_mirror_view_from_output(wo, show_animation, false);
        }
    }
    ...
    void ensure_mirror_view(wf::output_t* wo)
    {
        ...
        auto mirror = new wf_move_mirror_view(view, wo,
            base_output.x - mirror_output.x, base_output.y - mirror_output.y);

        wf::get_core().add_view(std::unique_ptr<wf::view_interface_t>(mirror));
     ...
     *
     * Views in wayfire are visible on only a single output. However, when the
     * user moves the view between outputs, it is desirable to temporarily show
     * the view on all outputs whose boundaries it crosses. We emulate this
     * behavior by creating mirror views of the view being moved, while fading
