...
void init()
{
    ...
    program.compile(default_vertex_shader_source, default_fragment_shader_source);

    color_program.set_simple(compile_program(
...
void render_transformed_texture(wf::texture_t tex, const gl_geometry& g,
    const gl_geometry& texg, glm::mat4 model, glm::vec4 color, uint32_t bits)
{
...
void render_transformed_texture(wf::texture_t texture,
    const wf::geometry_t& geometry, glm::mat4 transform, glm::vec4 color,
    uint32_t bits)
{
...
void render_texture(wf::texture_t texture, const wf::framebuffer_t& framebuffer,
...
void render_rectangle(
    wf::geometry_t geometry, wf::color_t color, glm::mat4 matrix)
{
...
void render_begin(int32_t viewport_width, int32_t viewport_height, uint32_t fb)
{
    ...
    wlr_renderer_begin(
        wf::get_core_impl().renderer, viewport_width, viewport_height);
...
bool wf::framebuffer_base_t::allocate(int width, int height)
{
    ...
    /* Special case: fb = 0. This occurs in the default workspace streams, we
...
void wf::framebuffer_base_t::scissor(wlr_box box) const
{
    ...
    GL_CALL(glScissor(
        box.x, viewport_height - box.y - box.height, box.width, box.height));
}
...
{
    auto ortho =
        glm::ortho(1.0f * geometry.x, 1.0f * geometry.x + 1.0f * geometry.width,
            1.0f * geometry.y + 1.0f * geometry.height, 1.0f * geometry.y);
...
glm::mat4 get_output_matrix_from_transform(wl_output_transform transform)
{
    ...
    if (rotation == WL_OUTPUT_TRANSFORM_180)
        rotation_matrix =
            glm::rotate(rotation_matrix, glm::radians(180.0f), {0, 0, 1});
    if (rotation == WL_OUTPUT_TRANSFORM_270)
        rotation_matrix =
            glm::rotate(rotation_matrix, glm::radians(270.0f), {0, 0, 1});
...
wf::texture_t::texture_t(wlr_texture* texture)
{
    ...
    if (this->target == GL_TEXTURE_2D) {
        this->type =
            attribs.has_alpha ? wf::TEXTURE_TYPE_RGBA : wf::TEXTURE_TYPE_RGBX;
    } else {
...
std::map<wf::texture_type_t, texture_type_builtins> builtins = {
    ...
    {wf::TEXTURE_TYPE_EXTERNAL,
        {builtin_external_source, builtin_ext_external_source}},
};

void program_t::compile(
    const std::string& vertex_source, const std::string& fragment_source)
{
    free_resources();

    for (const auto& program_type : builtins) {
        auto fragment = replace_builtin_with(
            fragment_source, builtin, program_type.second.builtin);
        fragment = replace_builtin_with(
            fragment, builtin_ext, program_type.second.builtin_ext);
...
void program_t::use(wf::texture_type_t type)
{
    if (priv->id[type] == 0) {
        throw std::runtime_error(
            "program_t has no program for type " + std::to_string(type));
    }
...
void program_t::attrib_pointer(
    const std::string& attrib, int size, int stride, const void* ptr, GLenum type)
{
