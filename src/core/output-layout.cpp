...
std::pair<wlr_output_mode, bool> parse_output_mode(std::string modeline)
{
    ...
    mode.refresh = 0;
    int read = std::sscanf(modeline.c_str(), "%d x %d @ %d", &mode.width,
...
wlr_output_mode* find_matching_mode(
    wlr_output* output, const wlr_output_mode& reference)
{
    wlr_output_mode* mode;
...
static bool parse_modeline(const char* modeline, drmModeModeInfo& mode)
{
    ...
    if (sscanf(modeline, "%f %hd %hd %hd %hd %hd %hd %hd %hd %15s %15s", &fclock,
    ...
    if (to) {
        for (auto& view : views) {
            ...
            to->workspace->move_to_workspace(
                view, to->workspace->get_current_workspace());
            ...
            if (!view->fullscreen && !view->tiled_edges && view->is_mapped()) {
                auto geometry = wf::clamp(
                    view->get_wm_geometry(), to->workspace->get_workarea());
...
struct output_layout_output_t
{
    ...
    std::shared_ptr<wf::config::option_base_t> mode_opt, position_opt, scale_opt,
        transform_opt;
        ...
        auto add_if_missing = [&](std::string name, std::string defval) {
            if (!section->get_option_or(name)) {
                section->register_new_option(
                    std::make_shared<wf::config::option_t<std::string>>(
                        name, defval));
            }
    ...
    output_layout_output_t(wlr_output* handle)
    {
        ...
        if (is_nested_compositor) {
             ...
             * these cases */
            on_mode.set_callback([=](void* data) { handle_mode_changed(); });
    ...
    wlr_output_mode select_default_mode()
    {
        ...
        wl_list_for_each_reverse(mode, &handle->modes, link) return *mode;
        ...
        if (set_mode != "default" && set_mode != "auto") {
            ...
            if (value.second) {
                if (is_mode_supported(value.first)) {
                    ...
                    LOGE("Output mode ", set_mode, " for output ", handle->name,
                        " is not supported, ", " try adding a custom mode.");
                }
            }
        }
    ...
    output_state_t load_state_from_config()
    {
        ...
        auto set_scale =
            wf::option_type::from_string<double>(scale_opt->get_value_str());
        if (set_scale.value_or(-1) <= 0) {
            LOGE("Invalid scale for ", handle->name,
                " in config: ", scale_opt->get_value_str());
            state.scale = 1;
        } else {
        ...
        state.transform =
            get_transform_from_string(transform_opt->get_value_str());
        return state;
    }
    ...
    void ensure_wayfire_output(const wf::dimensions_t& effective_size)
    {
        ...
        wlr_output* focused = get_core().get_active_output() ?
                                  get_core().get_active_output()->handle :
                                  nullptr;
    ...
    void apply_mode(const wlr_output_mode& mode)
    {
        ...
        if (built_in) {
            ...
            wlr_output_set_custom_mode(
                handle, mode.width, mode.height, mode.refresh);
        }
    ...
    void render_output(wlr_texture* texture)
    {
        ...
        wlr_matrix_project_box(
            box, &geometry, WL_OUTPUT_TRANSFORM_NORMAL, 0.0, projection);
    ...
    {
        auto wo =
            get_core().output_layout->find_output(current_state.mirror_from);
        if (!wo) {
        ...
        auto texture = wlr_texture_from_dmabuf(get_core().renderer, &attributes);
    ...
    wf::dimensions_t get_effective_size()
    {
        ...
        wlr_output_effective_resolution(
            handle, &effective_size.width, &effective_size.height);
...
class output_layout_t::impl
{
  ...
  public:
    impl(wlr_backend* backend)
    {
        ...
        output_pw_manager =
            wlr_output_power_manager_v1_create(get_core().display);
        ...
        for (auto& entry : config) {
                ...
                !entry.second.automatic_positioning)
            {
                ...
                wlr_output_layout_add(
                    output_layout, handle, state.position.x, state.position.y);
                lo->apply_state(state, shutdown_received);
            }
        }
    ...
    void send_wlr_configuration()
    {
        ...
        wlr_output_manager_v1_set_configuration(
            output_manager, wlr_configuration);
    }
    ...
    void set_power_mode(wlr_output_power_v1_set_mode_event* ev)
    {
        ...
        config[ev->output].source =
            (ev->mode == ZWLR_OUTPUT_POWER_V1_MODE_ON ? OUTPUT_IMAGE_SOURCE_SELF :
                                                        OUTPUT_IMAGE_SOURCE_DPMS);
    ...
    wf::output_t* get_output_coords_at(
        const wf::pointf_t& origin, wf::pointf_t& closest)
    {
        wlr_output_layout_closest_point(
            output_layout, NULL, origin.x, origin.y, &closest.x, &closest.y);
...
wf::output_t* output_layout_t::get_output_coords_at(
    wf::pointf_t origin, wf::pointf_t& closest)
{
