...
{
    LOGI(
        "handle new input: ", dev->name, ", default mapping: ", dev->output_name);
...
void input_manager::refresh_device_mappings()
{
    for (auto& device : this->input_devices) {
        ...
        std::string mapped_output = mapped_output_opt ?
                                        mapped_output_opt->get_value_str() :
                                        nonull(dev->output_name);
...
input_manager::input_manager()
{
    ...
    surface_map_state_changed = [=](wf::signal_data_t* data) {
        ...
        if (our_touch) {
            ...
            for (auto f : touch_points) {
                our_touch->gesture_recognizer.update_touch(
    ...
    wf::get_core().connect_signal(
        "_surface_unmapped", &surface_map_state_changed);
...
input_manager::~input_manager()
{
    ...
    wf::get_core().disconnect_signal(
        "_surface_mapped", &surface_map_state_changed);
    wf::get_core().disconnect_signal(
        "_surface_unmapped", &surface_map_state_changed);
...
void input_manager::ungrab_input()
{
    ...
    idle_update_cursor.run_once([&]() { lpointer->set_enable_focus(true); });
}
...
wf::surface_interface_t* input_manager::input_surface_at(
    wf::pointf_t global, wf::pointf_t& local)
{
    auto output =
        wf::get_core().output_layout->get_output_coords_at(global, global);
    /* If the output at these coordinates was just destroyed or some other edge
     * case
     */
    if (!output)
        return nullptr;
...
wf::binding_t* input_manager::new_binding(wf_binding_type type,
    std::shared_ptr<wf::config::option_base_t> value, wf::output_t* output,
    void* callback)
{
...
{
    rem_binding(
        [=](wf::binding_t* binding) { return binding->output == output; });
}
...
bool input_manager::check_button_bindings(uint32_t button)
{
    ...
    for (auto& binding : bindings[WF_BINDING_BUTTON]) {
        auto as_button =
            std::dynamic_pointer_cast<wf::config::option_t<wf::buttonbinding_t>>(
                binding->value);
        assert(as_button);
...
wf::SurfaceMapStateListener::SurfaceMapStateListener()
{
    ...
    wf::get_core().connect_signal(
        "_surface_mapped", &on_surface_map_state_change);
    wf::get_core().connect_signal(
        "_surface_unmapped", &on_surface_map_state_change);
}
