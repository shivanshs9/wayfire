...
void input_manager::create_seat()
{
    ...
    lpointer = std::make_unique<wf::LogicalPointer>(nonstd::make_observer(this));
    ...
    request_set_selection.set_callback([&](void* data) {
        ...
        wlr_seat_set_selection(
            wf::get_core().get_current_seat(), ev->source, ev->serial);
    });
...
wf::pointf_t get_surface_relative_coords(
    wf::surface_interface_t* surface, const wf::pointf_t& point)
{
