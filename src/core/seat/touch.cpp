...
void wf_gesture_recognizer::continue_gesture()
{
    ...
    bool is_left_swipe = true, is_right_swipe = true, is_up_swipe = true,
         is_down_swipe = true;
    ...
    if (swipe_dir) {
        ...
        bool bottom_edge = false, upper_edge = false, left_edge = false,
             right_edge = false;
        ...
        for (auto f : current) {
            ...
            upper_edge |= (f.second.start.y <= og.y + EDGE_SWIPE_THRESHOLD);
            left_edge |= (f.second.start.x <= og.x + EDGE_SWIPE_THRESHOLD);
            right_edge |=
...
void wf_gesture_recognizer::update_touch(
    int32_t time, int id, wf::pointf_t point, bool real_update)
{
...
wf_touch::wf_touch(wlr_cursor* cursor)
{
    on_down.set_callback([&](void* data) {
        ...
        wlr_cursor_absolute_to_layout_coords(
            wf::get_core_impl().input->cursor->cursor, ev->device, ev->x, ev->y,
            &lx, &ly);
...
void input_manager::set_touch_focus(
    wf::surface_interface_t* surface, uint32_t time, int id, wf::pointf_t point)
{
    ...
    if (id == 0) {
        auto compositor_surface =
            wf::compositor_surface_from_surface(touch_focus);
...
void input_manager::handle_touch_down(
    uint32_t time, int32_t id, wf::pointf_t point)
{
    mod_binding_key = 0;
...
void input_manager::handle_touch_motion(
    uint32_t time, int32_t id, wf::pointf_t point, bool real_update)
{
    if (active_grab) {
        auto wo = wf::get_core().output_layout->get_output_at(point.x, point.y);
        auto og = wo->get_layout_geometry();
        if (active_grab->callbacks.touch.motion && real_update) {
            active_grab->callbacks.touch.motion(
                id, point.x - og.x, point.y - og.y);
        }
...
void input_manager::handle_gesture(wf::touchgesture_t g)
{
    ...
    for (auto& binding : bindings[WF_BINDING_GESTURE]) {
        auto as_gesture =
            std::dynamic_pointer_cast<wf::config::option_t<wf::touchgesture_t>>(
                binding->value);
        assert(as_gesture);
...
{
    for (auto& f : gesture_recognizer.current) {
        wf::get_core_impl().input->set_touch_focus(
            nullptr, wf::get_current_time(), f.first, {0, 0});
    }
}
