...
void wf::LogicalPointer::update_cursor_position(
    uint32_t time_msec, bool real_update)
{
...
void wf::LogicalPointer::update_cursor_focus(
    wf::surface_interface_t* focus, wf::pointf_t local)
{
    ...
    if (focus && !compositor_surface_from_surface(focus)) {
        ...
        wlr_seat_pointer_notify_enter(
            input->seat, next_focus_wlr_surface, local.x, local.y);
    } else {
...
{
    auto view = (wf::view_interface_t*)(this->cursor_focus->get_main_surface());
...
static double distance_between_points(
    const wf::pointf_t& a, const wf::pointf_t& b)
{
    return std::sqrt(
        1.0 * (a.x - b.x) * (a.x - b.x) + 1.0 * (a.y - b.y) * (a.y - b.y));
}

static wf::pointf_t region_closest_point(
    const wf::region_t& region, const wf::pointf_t& ref)
{
...
void wf::LogicalPointer::send_button(
    wlr_event_pointer_button* ev, bool has_binding)
{
    ...
    wlr_seat_pointer_notify_button(
        input->seat, ev->time_msec, ev->button, ev->state);
}
...
void wf::LogicalPointer::send_motion(uint32_t time_msec, wf::pointf_t local)
{
    ...
    auto compositor_surface = compositor_surface_from_surface(this->cursor_focus);
    if (compositor_surface) {
        compositor_surface->on_pointer_motion(local.x, local.y);
    } else {
        wlr_seat_pointer_notify_motion(input->seat, time_msec, local.x, local.y);
    }
}
...
void wf::LogicalPointer::handle_pointer_motion(wlr_event_pointer_motion* ev)
{
    ...
    wlr_relative_pointer_manager_v1_send_relative_motion(
        ...
        (uint64_t)ev->time_msec * 1000, ev->delta_x, ev->delta_y, ev->unaccel_dx,
        ev->unaccel_dy);
    ...
    wlr_event_pointer_motion_absolute* ev)
{
    ...
    wlr_cursor_absolute_to_layout_coords(
        input->cursor->cursor, ev->device, ev->x, ev->y, &cx, &cy);
    ...
    wlr_event_pointer_swipe_begin* ev)
{
    wlr_pointer_gestures_v1_send_swipe_begin(
        wf::get_core().protocols.pointer_gestures, input->seat, ev->time_msec,
        ev->fingers);
}
...
void wf::LogicalPointer::handle_pointer_swipe_end(wlr_event_pointer_swipe_end* ev)
{
    ...
    wlr_event_pointer_pinch_update* ev)
{
    wlr_pointer_gestures_v1_send_pinch_update(
        wf::get_core().protocols.pointer_gestures, input->seat, ev->time_msec,
        ev->dx, ev->dy, ev->scale, ev->rotation);
}

void wf::LogicalPointer::handle_pointer_pinch_end(wlr_event_pointer_pinch_end* ev)
{
