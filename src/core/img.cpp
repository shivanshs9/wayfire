...
using Writer = std::function<void(
    const char* name, uint8_t* pixels, unsigned long, unsigned long)>;
namespace
{
...
bool texture_from_png(const char* filename, GLuint target)
{
    ...
    GL_CALL(glTexImage2D(target, 0, GL_RGBA, width, height, 0, GL_RGBA,
...
bool texture_from_jpeg(const char* FileName, GLuint target)
{
    ...
    while (infot.output_scanline < infot.output_height) {
        rowptr[0] = (unsigned char*)jdata +
                    3 * infot.output_width * infot.output_scanline;
        jpeg_read_scanlines(&infot, rowptr, 1);
    }

    jpeg_finish_decompress(&infot);

    GL_CALL(glTexImage2D(target, 0, GL_RGB, infot.output_width,
        infot.output_height, 0, GL_RGB, GL_UNSIGNED_BYTE, jdata));
...
void write_to_file(
    std::string name, uint8_t* pixels, int w, int h, std::string type)
{
