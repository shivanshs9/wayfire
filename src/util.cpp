...
{
    stream << std::fixed << std::setprecision(4) << '(' << pointf.x << ','
           << pointf.y << ')';
    return stream;
}
...
wf::geometry_t wf::geometry_intersection(
    const wf::geometry_t& r1, const wf::geometry_t& r2)
{
    wlr_box result;
...
bool wf::region_t::contains_point(const wf::point_t& point) const
{
    return pixman_region32_contains_point(
        this->unconst(), point.x, point.y, NULL);
}
...
wf::region_t wf::region_t::operator&(const wf::region_t& other) const
{
    ...
    pixman_region32_intersect(
        result.to_pixman(), this->unconst(), other.unconst());
...
{
    pixman_region32_intersect_rect(this->to_pixman(), this->to_pixman(), box.x,
...
{
    pixman_region32_intersect(
        this->to_pixman(), this->to_pixman(), other.unconst());
    return *this;
}
...
{
    wf::region_t result;
    pixman_region32_union_rect(result.to_pixman(), this->unconst(), other.x,
        other.y, other.width, other.height);
    return result;
}
...
{
    pixman_region32_union_rect(this->to_pixman(), this->to_pixman(), other.x,
        other.y, other.width, other.height);
    return *this;
}
...
wf::region_t wf::region_t::operator^(const wlr_box& box) const
{
    ...
    pixman_region32_subtract(
        result.to_pixman(), this->unconst(), sub.to_pixman());
    return result;
}
...
wf::region_t& wf::region_t::operator^=(const wlr_box& box)
{
    wf::region_t sub{box};
    pixman_region32_subtract(
        this->to_pixman(), this->to_pixman(), sub.to_pixman());
    return *this;
}
...
wf::geometry_t wf::clamp(wf::geometry_t window, wf::geometry_t output)
{
    ...
    window.x =
        wf::clamp(window.x, output.x, output.x + output.width - window.width);
    window.y =
        wf::clamp(window.y, output.y, output.y + output.height - window.height);
...
void wl_timer::set_timeout(uint32_t timeout_ms, callback_t call)
{
    ...
    if (!source)
        source =
            wl_event_loop_add_timer(get_core().ev_loop, handle_timeout, this);
