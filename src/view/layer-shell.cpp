  ...
  private:
    ...
    wf_layer_shell_manager()
    {
        wf::get_core().output_layout->connect_signal(
            "configuration-changed", &on_output_layout_changed);
        ;
    }
  ...
  public:
    ...
    void set_exclusive_zone(wayfire_layer_shell_view* v)
    {
        ...
        if (!v->anchored_area) {
            ...
            v->anchored_area->reflowed = [v](wf::geometry_t geometry,
                                             wf::geometry_t _) {
        ...
        v->anchored_area->real_size =
            v->anchored_area->edge <=
                    wf::workspace_manager::ANCHORED_EDGE_BOTTOM ?
                v->lsurface->current.desired_height :
                v->lsurface->current.desired_width;
    }
    ...
    void pin_view(wayfire_layer_shell_view* v, wf::geometry_t usable_workarea)
    {
        ...
        auto bounds = v->lsurface->current.exclusive_zone < 0 ?
                          v->get_output()->get_relative_geometry() :
                          usable_workarea;
    ...
    void arrange_layers(wf::output_t* output)
    {
        ...
        focused_layer_request_uid =
            wf::get_core().focus_layer(focus_mask, focused_layer_request_uid);
...
{
    LOGD("Create a layer surface: namespace ", lsurf->namespace_t, " layer ",
        lsurf->current.layer);
...
void wayfire_layer_shell_view::initialize()
{
    ...
    on_new_popup.set_callback(
        [&](void* data) { create_xdg_popup((wlr_xdg_popup*)data); });
...
wf::layer_t wayfire_layer_shell_view::get_layer()
{
    ...
    auto it = std::find(desktop_widget_ids.begin(), desktop_widget_ids.end(),
        nonull(lsurface->namespace_t));
